...
# ----------
# Split video files with mencoder.
#
# Usage
# ~~~~~
#
# msplitter {-n number | -l length | -p position1,position2...} \
...
# ~~~~
#
# 1. You must specified split method and file, method can be number, length or
# position, number and can use together.
#
# 2. Number must a integer. Length is a time format, for example, two minutes
# can express by "120" or "2:0" or "1:60". position also follow by time format
# like length, but it can separate by comma.
#
# 3. Deafult filetype is avi. default ouput folder is working folder, default
# mencoder arguments is "-oac copy -ovc copy".
#
# 4. If "-m" is not given, just print the command, you can check it, or
# reditect it to file for edit. If is given, it will directly call mencoder to
# split files.
#
# ----------
